...
import { ABI } from "../resolvers/Rinkeby/0x61E81C030b662C12eb56Bc6883E4382Ece07906c";
...
export default withStyles(styles)(function DAppStore({ classes, ein }) {
  ...
  const [hydroSnackbar, dispatchHydroSnackbar] = useReducer(
    hydroSnackbarReducer,
    initialHydroSnackbar
  );
  ...
  const factory = useGenericContract(
    "0x728EfdDFf9B5C444b3f486374c8d2724332a87E8",
    ABI
  );
  ...
  const recognizedResolverIndex = allResolvers.findIndex(
    e => e === resolverAddress
  );
      ...
      {!ready ? (
        ...
        <>
              ...
              >
                <DialogContent>
                  <Suspense fallback={<div />}>
                    {React.createElement(
                      resolverDetails[recognizedResolverIndex]
                        .extraDataComponent,
          ...
          <TransactionButton
            show={
              resolverAddress !== "" &&
              context.library.utils.checkAddressChecksum(resolverAddress)
            }
          ...
          <div className={classes.root}>
            ...
            >
              {allResolvers.map((resolver, i) => (
                <GridListTile
                  ...
                  onClick={() => {
                    if (einDetails.resolvers.includes(resolver)) {
                      ...
                      Number(snowflakeBalance) <
                      Number(resolverDetails[i].requiredAllowance)
                    ) {
                      setResolverAddress("");
                      setAllowance("");
                      dispatchHydroSnackbar({
                        type: "show",
                        allowance: resolverDetails[i].requiredAllowance
                      });
                    } else {
                ...
                >
                  <img
                    src={resolverDetails[i].logo}
                    alt={resolverDetails[i].name}
                  />
                  <GridListTileBar
                    ...
                    actionIcon={
                      <DoneIcon
                        style={
                          einDetails.resolvers.includes(resolver)
                            ? {}
                            : { display: "none" }
                        }
          ...
          <div>
            <Snackbar
              ...
              onClose={(e, reason) =>
                reason === "clickaway" ? null : setAlreadySetSnackbar(true)
              }
              ContentProps={{
              ...
              message={
                <span id="message-id">
                  You have already added this resolver.
                </span>
              }
              action={[
                <IconButton
            ...
            <Snackbar
              ...
              onClose={(e, reason) =>
                reason === "clickaway"
                  ? null
                  : dispatchHydroSnackbar({ type: "hide" })
              }
            >
              <SnackbarContent
                ...
                message={
                  <p>
                    Insufficient Balance. Required: {hydroSnackbar.allowance}{" "}
                    HYDRO.
                  </p>
                }
                action={[
