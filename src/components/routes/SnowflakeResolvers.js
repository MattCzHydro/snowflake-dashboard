...
export default withRouter(
  withStyles(styles)(function SnowflakeResolvers({
    classes,
    resolvers,
    resolverDetails,
    resolverAllowances,
    ...
    const [selectedResolvers, dispatchSelected] = useReducer(
      selectedReducer,
      Array(Object.keys(resolverAllowances).length).fill(false)
    );
    const [newAllowances, dispatchAllowance] = useReducer(
      allowancesReducer,
      resolverAllowances
    );
    ...
    return (
      <div style={{ width: "100%" }}>
        <Typography>
          Manage Resolvers linked to your 1484 Snowflake Identity.
        </Typography>
        ...
        <Toolbar style={{ visibility: anySelected ? "visible" : "hidden" }}>
          <TransactionButton
            ...
            method={() =>
              snowflakeContract.methods.changeResolverAllowances(
                resolvers.filter((r, i) => selectedResolvers[i]),
                newAllowances
                  .filter((a, i) => selectedResolvers[i])
                  .map(a => fromDecimal(a, 18))
              )
            }
              ...
              <TableCell padding="checkbox">
                {resolvers.length === 0 ? (
                  ...
                  <Checkbox
                    ...
                    onChange={() => {
                      dispatchSelected({
                        type: allSelected ? "toggleAllOff" : "toggleAllOn"
                      });
                    }}
                  />
                )}
          ...
          <TableBody>
            {resolvers.map((resolver, i) => (
              ...
              >
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedResolvers[i]}
                    onClick={() => handleCheckboxClick(resolver, i)}
                  />
                </TableCell>
                <TableCell>
                  <a
                ...
                <TableCell padding="checkbox">
                  {resolver !== clientRaindropAddress && (
                    <TransactionButton
                      ...
                      method={() =>
                        snowflakeContract.methods.removeResolver(
                          resolver,
                          true,
                          "0x00"
                        )
