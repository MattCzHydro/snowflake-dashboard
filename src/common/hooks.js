...
export function useGenericContract(address, ABI) {
  ...
  return useMemo(() => new context.library.eth.Contract(ABI, address), [
    address,
    ABI
  ]);
}
...
export function useEIN(address) {
  ...
  useAccountEffect(() => {
    _1484Contract.methods
      .getEIN(address || context.account)
      .call()
      .then(result => {
        if (
          result ===
...
export function useHydroId() {
  ...
  const [hydroId, setHydroId] = useState({
    hydroId: undefined,
    hydroIdAddress: undefined
  });

  useAccountEffect(() => {
    if (ein) {
      clientRaindropContract.methods["getDetails(uint256)"](ein)
        .call()
        .then(result =>
          setHydroId({
            hydroId: result.casedHydroID,
            hydroIdAddress: result._address
          })
        )
  ...
  if (ein === null) return [null, null];
    ...
    return hydroId === null
      ? [null, null]
      : [hydroId.hydroId, hydroId.hydroIdAddress];
}
      ...
      snowflakeContract.methods
        ...
        .then(balance => {
          setSnowflakeBalance(
            unconverted
              ? balance
              : Number(toDecimal(balance, 18)).toLocaleString(undefined, {
    ...
    if (ein && resolvers.length > 0) {
      Promise.all(
        resolvers.map(resolver =>
          snowflakeContract.methods
            .resolverAllowances(ein, resolver)
            .call()
...
async function getResolverDetails(
  web3js,
  snowflakeContract,
  networkName,
  resolver
) {
  const genericContract = new web3js.eth.Contract(
  ...
  const description = () =>
    genericContract.methods.snowflakeDescription().call();
...
export function useSessionStorageState(defaultValue, key) {
  ...
  if (firstRender.current) {
    ...
    if (potentialSessionStorageState)
      initialSessionStorageState = JSON.parse(potentialSessionStorageState);
  }

  const [sessionStorageState, setSessionStorageState] = useState(
    initialSessionStorageState
  );
